...
- The paper's stated motivation for replacing the single judge is that a single evaluator model introduces intra-model bias. ^s2
...
- Across the single-hop question answering datasets the panel correlates most strongly with human judgment, while the largest single model is among the weaker evaluators in that setting. ^s5
...
- Measured against human annotators on the multi-hop datasets, the panel deviates least, with a standard deviation of 2.2 against 6.1 for the widest-spreading single judge. ^s6
  > "We observe that overall, PoLL has the smallest spread in scores, with a standard deviation of 2.2, compared to EM and individual judges. GPT-3.5 has the highest spread, with a standard deviation of 6.1." (arXiv:2404.18796v2, p. 5)
- For each evaluated model, the largest positive deviation from human judgment occurs where that model is judged by itself. ^s7
...
- In the Chatbot Arena Hard ranking, the single large judge places a variant of its own family two positions above that variant's actual rank. ^s8
...
- The evaluation covers three judge settings and a limited number of judges and panel compositions, and the selection of panel members is left to further work. ^s10
