...
Under the practice term context engineering, the assembly of what reaches the model is handled as work with a stated goal. The term is defined as the set of strategies for curating and maintaining the optimal set of tokens during inference, counting the information that lands in the context outside the prompts, and the criterion of doing it well is stated as the smallest possible set of high-signal tokens that maximises the likelihood of the desired outcome. The curation phase recurs each time something is passed to the model, so the assembly is a repeated decision under that criterion. A controlled experiment supplies the empirical subject matter of such a decision, because model performance degrades significantly when the position of the relevant information within the input context changes, and because a longer input context trades the added information against the added content the model must reason over.
